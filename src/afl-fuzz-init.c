...
void perform_dry_run(afl_state_t *afl) {
  ...
  for (idx = 0; idx < afl->queued_paths; idx++) {
    ...
    if (afl->use_seamfuzz != 0 || afl->use_MesFuzz != 0) {//这里
      update_hit_table(afl);
      
      u8 *tmp_fname = alloc_printf("%s", fn);
      u8 *tmp_fn_id = strtok(tmp_fname, ":");
      tmp_fn_id = strtok(NULL, ",");

      u8 *tmp_fn = alloc_printf("%s/%d", afl->covered_path_dir, atoi(tmp_fn_id));
...
static void handle_existing_out_dir(afl_state_t *afl) {
  ...
  /*MesFuzz清空mutation_select文件*/
  if (!afl->in_place_resume) {

    fn = alloc_printf("%s/mutation_select", afl->out_dir);
    if (unlink(fn) && errno != ENOENT) { goto dir_cleanup_failed; }
    ck_free(fn);
...
void setup_dirs_fds(afl_state_t *afl) {
  ...
  if (!afl->in_place_resume) {
    ...
    fprintf(
        ...
        "unique_hangs, max_depth, execs_per_sec, total_execs, edges_found, # dist_call, # grouping, # group, # deleted, run_times \n");
  ...
  /* MesFuzz创建突变选择的记录文件 */
  tmp = alloc_printf("%s/mutation_select", afl->out_dir);
  int fd = open(tmp, O_WRONLY | O_CREAT | O_EXCL, DEFAULT_PERMISSION);
  if (fd < 0) { PFATAL("Unable to create '%s'", tmp); }
  ck_free(tmp);
  afl->mutation_select = fdopen(fd, "w");
  if (!afl->mutation_select) { PFATAL("fdopen() failed"); }
  fprintf(afl->mutation_select, "The code of mutation operator:\n");
