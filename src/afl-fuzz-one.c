/*
   ...
   Copyright 2019-2020 AFLplusplus Project. All rights reserved.o
...
u8 fuzz_one_seam(afl_state_t *afl) {
  struct queue_entry* current_working_file;
  ...
  current_working_file = afl->queue_cur;
  u8* current_working_fname = alloc_printf("%s", current_working_file->fname);

  u8* current_working_full_id = alloc_printf("%s", current_working_fname);
  u8* current_working_id;

  current_working_id = strtok(current_working_full_id, ":");
  current_working_id = strtok(NULL, ",");
  
  afl->current_working_int_id = atoi(current_working_id);

  ck_free(current_working_full_id);
  ck_free(current_working_fname);
  ...
  if (unlikely(afl->custom_mutators_count)) {//是否有自定义的突变策略？
  ...
  if (likely(afl->pending_favored)) {
    ...
    /* If we have any favored, non-fuzzed new arrivals in the queue,
       possibly skip to them at the expense of already-fuzzed or non-favored
       cases. 如果队列中有未处理的favored的对象，则大概率跳过当前的对象*/
  ...
  afl->subseq_tmouts = 0;//Number of timeouts in a row

  afl->cur_depth = afl->queue_cur->depth;

  /*******************************************
   * CALIBRATION (only if failed earlier on) 
   CALIBRATION（校准）（仅在先前失败的情况下）*
   *******************************************/
  ...
  if (unlikely(!afl->non_instrumented_mode && !afl->queue_cur->trim_done &&
    ...
    u8 res = trim_case(afl, afl->queue_cur, in_buf);//从这开始出现测试界面，trim_case的作用是修建测试用例，减小其大小
    orig_in = in_buf = queue_testcase_get(afl, afl->queue_cur);//queue_testcase_get()首先判断是否启用缓存，如果未启用缓存，则从文件中读入数据并返回；否则，判断该测试用例是否已经缓存到内存中，如果已被缓存，则直接从内存中取出即可，否则将该测试用例缓存到内存中，返回指针afl->queue_cur->testcase_buf。
...
/* maintains 21 mutation operators, from original AFL, in havoc stage  */
#define MAX_HAVOC_ENTRY_SEAM 21                                      

  u32 r_max, r;

  r_max = (MAX_HAVOC_ENTRY_SEAM + 1) + (afl->extras_cnt ? 2 : 0) + (afl->a_extras_cnt ? 2 : 0);//afl->extras_cnt为字典中tokens的总数

  u8 use_existing_prob2 = 0;
  afl->use_existing_prob = 0;

  /* We essentially just do several thousand runs (depending on perf_score)
     where we take the input file and make random stacked tweaks. */

  afl->max_op_num = r_max;

  if (afl->is_splice == 0) {//如果不是splice阶段，所以不管splice阶段？
    if (afl->prob_group_cnt > 1) {   //概率分组的数量？   
      sorting_hit_table(afl);
         
      afl->dist_cnt++;

      afl->prob_group_id = seed_cluster(afl, out_buf, len);//返回种子聚类的分组

      if (afl->prob_group_id == UINT_MAX) {//产生了新的种子聚类
        afl->prob_group_id = afl->prob_group_cnt;
        if (afl->use_thompson == 1) print_thompson_info(afl, afl->prob_group_id, 0);
        else update_prob_using_group(afl, afl->prob_group_id, 0);
        afl->group_cnt[afl->prob_group_id]++;

      } else {
        use_existing_prob2 = 1;
        afl->grouping_cnt++;

        if (afl->use_thompson == 1) print_thompson_info(afl, afl->prob_group_id, 1);
        else update_prob_using_group(afl, afl->prob_group_id, 1);
        
        afl->group_cnt[afl->prob_group_id]++;
      }

      if (afl->use_seamfuzz_debuging == 1) {
        print_seed_in_swarm_in_file(afl);
        print_updated_probability(afl, afl->prob_group_id);
      }

      afl->group_exists[afl->current_working_int_id] = afl->prob_group_id;

    }

  }

  for (afl->stage_cur = 0; afl->stage_cur < afl->stage_max; ++afl->stage_cur) {//进行多少轮次的havoc突变？

    u32 use_stacking = 1 << (1 + rand_below(afl, afl->havoc_stack_pow2));

    if (use_existing_prob2 == 1) {
      if (rand_below(afl, 100) > afl->explore_prob) {
        afl->use_existing_prob = 1;
      }
      else {
        afl->use_existing_prob = 0;
      }
    ...
    afl->stage_cur_val = use_stacking;
    afl->havoc_trials = use_stacking;

    memset(afl->op_trace, 50, use_stacking);
    memset(afl->sp_trace, sp_num, use_stacking);

    u32 end_point, current_place;
    ...
    for (i = 0; i < use_stacking; ++i) {//每一次havoc突变进行了几次操作？
      ...
      r = select_operator(afl, r_max);
      //r = DARWIN_SelectOperator();
      //if (r > 59) r = 60;
      
      //fprintf(afl->mutation_select,"%u, ", r);
      //fflush(afl->mutation_select);
      ...
      switch (r) {
        ...
        case 0: {
          ...
          strcat(afl->mutation, afl->m_tmp);
#endif
          end_point = temp_len << 3;
          current_place = calculate_starting_point(afl, end_point, i);

          FLIP_BIT(out_buf, current_place);
          break;
        }
        ...
        case 2: {
          ...
          strcat(afl->mutation, afl->m_tmp);
#endif
          end_point = temp_len - 1;
          current_place = calculate_starting_point(afl, end_point, i);
          
          *(u16 *)(out_buf + current_place) =
              interesting_16[rand_below(afl, sizeof(interesting_16) >> 1)];
        ...
        case 3: {
          ...
          *(u16 *)(out_buf + current_place) = SWAP16(
              interesting_16[rand_below(afl, sizeof(interesting_16) >> 1)]);
        ...
        case 5: {
          ...
          *(u32 *)(out_buf + current_place) = SWAP32(
              interesting_32[rand_below(afl, sizeof(interesting_32) >> 2)]);
        ...
        case 6: {
          ...
          out_buf[current_place] -= 1 + rand_below(afl, ARITH_MAX);
          break;
        ...
        case 8: {
          ...
          strcat(afl->mutation, afl->m_tmp);
#endif
          *(u16 *)(out_buf + current_place) -= 1 + rand_below(afl, ARITH_MAX);
...
#endif
          *(u16 *)(out_buf + current_place) =
              SWAP16(SWAP16(*(u16 *)(out_buf + current_place)) - num);
        ...
        case 12: {
          ...
          strcat(afl->mutation, afl->m_tmp);
#endif
          *(u32 *)(out_buf + current_place) -= 1 + rand_below(afl, ARITH_MAX);
...
#endif
          *(u32 *)(out_buf + current_place) =
              SWAP32(SWAP32(*(u32 *)(out_buf + current_place)) - num);
        ...
        case 16: {
          ...
          out_buf[current_place] ^= 1 + rand_below(afl, 255);
          break;
        ...
        case 18: {
          ...
          if (temp_len + HAVOC_BLK_XL < MAX_FILE) {
            ...
            u32 clone_to   = current_place;  // 0 ~ temp_len - 1)
        ...
        case 19: {
          ...
          end_point = temp_len - copy_len + 1;
          current_place = calculate_starting_point(afl, end_point, i);

          u32 copy_to   = current_place;
        ...
        case 21: {
          ...
          end_point = temp_len - del_len + 1;
          current_place = calculate_starting_point(afl, end_point, i); 

          u32 del_from = current_place;
        ...
        default:
          ...
          if (afl->extras_cnt) {
            ...
            if (r < 1) {
              ...
              end_point = temp_len - extra_len + 1;//这两行是作者加的
              current_place = calculate_starting_point(afl, end_point, i); //这两行是作者加的    

              u32 insert_at = current_place;
            ...
            } else if (r < 2) {//fuzz_one_original中是 r<4
    ...
    if (common_fuzz_stuff_seam(afl, out_buf, temp_len)) { goto abandon_entry; }
  ...
  if (afl->is_splice == 0) {
    ...
    if (afl->group_start == 0) {
      afl->file_table = ck_alloc(sizeof(struct cluster_table));
      afl->file_table->file_buf = afl->queue_cur->fname;
      afl->file_table->len = len;
      afl->file_table->group_id = afl->prob_group_cnt;
      afl->file_table->file_id = afl->current_working_int_id;
      afl->file_table->group_covered_path = alloc_printf("%s/%d", afl->group_covered_path_dir, afl->prob_group_cnt);
      
      afl->group_exists[afl->current_working_int_id] = afl->prob_group_cnt;

      if (afl->covered_path_dir == NULL) goto abandon_entry;//解决Unable to open '(null)/0'错误
      init_group_path(afl);

      afl->file_table_ori = afl->file_table;      

      afl->prob_group_cnt++;
      afl->group_start = 1;
    } else if (afl->make_new_group == 1) {
      struct cluster_table* tmp_file_table = ck_alloc(sizeof(struct cluster_table));

      tmp_file_table->file_buf = afl->queue_cur->fname;
      tmp_file_table->len = len;
      tmp_file_table->group_id = afl->prob_group_cnt;
      tmp_file_table->file_id = afl->current_working_int_id;
      tmp_file_table->group_covered_path = alloc_printf("%s/%d", afl->group_covered_path_dir, afl->prob_group_cnt);
      ...
      if(afl->file_table == NULL) afl->file_table = tmp_file_table;//解决段错误
      afl->file_table->next_candidate = tmp_file_table;
      afl->file_table = tmp_file_table;
...
u8 fuzz_one_MesFuzz(afl_state_t *afl,cluster_data_structures_for_AMBES cluster_evo_data_x[]) {
  struct queue_entry* current_working_file;
  ...
  if (unlikely(!afl->non_instrumented_mode && !afl->queue_cur->trim_done &&
    ...
    u8 res = trim_case(afl, afl->queue_cur, in_buf);
    orig_in = in_buf = queue_testcase_get(afl, afl->queue_cur);//queue_testcase_get()
    if (unlikely(res == FSRV_RUN_ERROR)) {
...
#define MAX_HAVOC_ENTRY_SEAM 21                                      
  ...
  r_max = (MAX_HAVOC_ENTRY_SEAM + 1) + (afl->extras_cnt ? 2 : 0) + (afl->a_extras_cnt ? 2 : 0);
  ...
  if (afl->is_splice == 0) {
    if (afl->prob_group_cnt >= 1) { 
      sorting_hit_table(afl);
         
      afl->dist_cnt++;

      afl->prob_group_id = seed_cluster(afl, out_buf, len);
      if (afl->prob_group_id == afl->prob_group_cnt) afl->prob_group_id = afl->prob_group_cnt - 1;

      if (afl->prob_group_id == UINT_MAX) {
        afl->prob_group_cnt++;
        afl->prob_group_id = afl->prob_group_cnt;
        //if (afl->use_thompson == 1) print_thompson_info(afl, afl->prob_group_id, 0);
        //else update_prob_using_group(afl, afl->prob_group_id, 0);
        ...
        mesfuzz_DAR_init(afl, afl->queued_at_start, afl->max_op_num, afl->prob_group_id, cluster_evo_data_x);
        afl->group_cnt[afl->prob_group_id]++;
  ...
  for (afl->stage_cur = 0; afl->stage_cur < afl->stage_max; ++afl->stage_cur) {
    ...
    for (i = 0; i < use_stacking; ++i) {
      ...
      r = MesFuzz_SelectOperator(0,afl->prob_group_id,cluster_evo_data_x);
        ...
        default:
          ...
          if (afl->a_extras_cnt) {
            ...
            if (r < 1) {
              ...
              end_point = temp_len - extra_len + 1;
              current_place = calculate_starting_point(afl, end_point, i);
    ...
    //unsigned out_of_has_new_bits = has_new_bits(afl, afl->virgin_bits);
    //out_of_has_new_bits = out_of_has_new_bits > (afl->queued_paths - havoc_queued)? out_of_has_new_bits:afl->queued_paths - havoc_queued;

    if (afl->queued_paths != havoc_queued) {
    /*if (out_of_has_new_bits >= 1) {*/
      if (perf_score <= afl->havoc_max_mult * 100) {
      ...
      //afl->queued_paths - havoc_queued;
      MesFuzz_NotifyFeedback(afl, 0, afl->queued_paths - havoc_queued,afl->prob_group_id,cluster_evo_data_x);
    
    }else MesFuzz_NotifyFeedback(afl, 0, afl->queued_paths - havoc_queued,afl->prob_group_id,cluster_evo_data_x);
  }
...
u8 fuzz_one(afl_state_t *afl,cluster_data_structures_for_AMBES cluster_evo_data_x[]) {
  ...
  if (afl->use_seamfuzz != 0) { 
    check_seamfuzz_dirs(afl);
    key_val_lv_1 = fuzz_one_seam(afl); 
    if (afl->use_seamfuzz_debuging == 1) print_group_perf(afl);
  }
  else if (afl->use_MesFuzz == 1) { 
    check_seamfuzz_dirs(afl);
    key_val_lv_1 = fuzz_one_MesFuzz(afl,cluster_evo_data_x); }
