...
//这里输出plot_data数据
  fprintf(afl->fsrv.plot_file,
          ...
          afl->plot_prev_ed, t_bytes, afl->dist_cnt, afl->grouping_cnt, afl->prob_group_cnt,afl->run_times);                     /* ignore errors */
...
#define IB(i) (val_buf[(i)])
  ...
  if (afl->use_seamfuzz != 0 ) {   /* When using seamfuzz mod... */
    
    sprintf(
        tmp + banner_pad, "%s " cLCY VERSION cLGN " (%s) " cPIN "[%s]",
        afl->crash_mode ? cPIN "SeamFuzz" : cYEL "american fuzzy lop SeamFuzz",
        afl->use_banner, afl->power_name);

  }
  else if (afl->use_MesFuzz != 0) {
    sprintf(
        tmp + banner_pad, "%s " cLCY VERSION cLGN " (%s) " cPIN "[%s]",
        afl->crash_mode ? cPIN "MesFuzz" : cYEL "american fuzzy lop MesFuzz",
        afl->use_banner, afl->power_name);
  } 
  else {
