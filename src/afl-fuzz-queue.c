...
void add_to_queue(afl_state_t *afl, u8 *fname, u32 len, u8 passed_det) {
  ...
  if (afl->queue_cnt > 0 && (afl->use_seamfuzz != 0 || afl -> use_MesFuzz != 0)) {
    u8 *tmp_fn = alloc_printf("%s/%d", afl->covered_path_dir, afl->queued_paths - 1);
    write_covered_path(afl, tmp_fn);
    write_time_while_find_paths(afl, afl->prob_group_id-1);
    afl->write_seed_cnt++;
    ck_free(tmp_fn);
  }
}
...
void destroy_seam(afl_state_t *afl) {
  struct cluster_table *tmp_group = afl->file_table_ori, *next_group;
  u8 *tmp_fn;

  while (tmp_group != NULL) {
    next_group = tmp_group->next_candidate;
    remove(tmp_group->group_covered_path);    
    
    ck_free(tmp_group->file_buf);
    ck_free(tmp_group->group_covered_path);    
    //ck_free(tmp_group->group_coverage);

    ck_free(tmp_group);
    tmp_group = next_group;
  }
  

  for (u32 i = 0; i < afl->write_seed_cnt; i++) {
    tmp_fn = alloc_printf("%s/%d", afl->covered_path_dir, i);
    //remove(tmp_fn);
    ck_free(tmp_fn);
  }
...
u32 calculate_score(afl_state_t *afl, struct queue_entry *q) {
  ...
  if (afl->use_MesFuzz == 1){
    u8 is_rare= check_hit_table (afl);
    if(is_rare == 1 && afl->rare_multiple) perf_score *= afl->rare_multiple;
  }
