...
static void usage(u8 *argv0, int more_help) {
  ...
  SAYF(
      ...
      "  -u            - Set the updating value for rewards \n"
       ...
       "MesFuzz settings:\n"////添加新参数，需要改这里
      "  -j      - enable MesFuzz \n"
      "  -r factor     - Multiple scoring of seeds if hit rare path"
      "  -P factor     - The proportion of structural similarity,functional similarity is 1-*"
      "  -g float      - Set the value for gamma (cluster), default value is 0.8 [0 ... 1.0] \n"
      ,
...
int main(int argc, char **argv_orig, char **envp) {
  ...
  afl->use_MesFuzz=0;
  cluster_data_structures_for_AMBES cluster_evo_data_x[200];
  ...
  while ((opt = getopt(
              argc, argv,
              "+b:B:c:CdDe:E:hi:I:f:F:l:L:m:M:nNOo:p:RQs:S:t:T:UV:Wx:ZlK:kA:a:g:X:ujr:P:")) > 0) {
    ...
    switch (opt) {
      ...
      case 'L': {                                              /* MOpt mode */
        ...
        if (afl->limit_time_sig) { FATAL("Multiple -L options no  t supported"); }
...
afl->use_MesFuzz = 1;
      ...
      case 'K':      // 1: original seamfuzz which uses our data classification (rare/common)
                     // 2: naive seamfuzz which does not use our data classificaiont (based on AFL++ criteria for interesting test-cases)
        if (sscanf(optarg, "%d", &afl->use_seamfuzz) < 1) {
        ...
        afl->alpha_val = 0.1;
        afl->beta_val = 0.9;
        //afl->gamma_val = 0.6;//种子之间的相似性为0.6，原本是0.8

        afl->prob_group_cnt = 1;
        afl->prob_group_id = 1;
        ...
        afl->reward = 1;
        afl->reward_update_value = 10;

        break;

      case 'k':      /* for debugging SeamFuzz */
      
        afl->use_seamfuzz_debuging = 1;
        

        break;

      //添加新参数，需要改这里
      case 'j':      /* for enable MesFuzz 这里*/
      
        afl->use_MesFuzz = 1;
        afl->use_MesFuzz_debuging = 1;
      ...
      case 'r':
        if (sscanf(optarg, "%d", &afl->rare_multiple) < 1) {

          FATAL("Bad syntax used for -r");
        
        }
        break;

      case 'P':
        if (sscanf(optarg, "%f", &afl->pro_stru_similarity) < 1) {
      ...
      case 'A':                                               /* thompson.py file path */

        if (afl->use_thompson == 1) { FATAL("Multiple -A options not supported"); }
        afl->thompson_path = optarg;
        afl->use_thompson = 1;
        break;
        
      case 'a':                   /* For syntactic similarity */
      
        if (sscanf(optarg, "%lf", &afl->alpha_val) < 1) {

          FATAL("Bad syntax used for -a");

        }

        afl->beta_val = 1.0 - afl->alpha_val;

        break;

      case 'g':                   /* For the threashold on seed clusteering。决定种子聚类的数量，数值越大，要求同一簇内的种子相似性越高，分类越多。默认为0.8 */
      
        if (sscanf(optarg, "%lf", &afl->gamma_val) < 1) {

          FATAL("Bad syntax used for -g");

        }
        
        printf("%f %f %f", afl->alpha_val, afl->beta_val, afl->gamma_val);

        break;

      case 'X':                   /* For the exploration probability */
      
        if (sscanf(optarg, "%d", &afl->explore_prob) < 1) {

          FATAL("Bad syntax used for -X");
      ...
      case 'u':                   /* For reward update value */
      
        if (sscanf(optarg, "%d", &afl->reward_update_value) < 1) {
  ...
  if (afl->use_seamfuzz != 0) setup_mesfuzz_dirs(afl);
  if (afl->use_MesFuzz != 0) setup_mesfuzz_dirs(afl);
  ...
  pivot_inputs(afl);//在输出文件夹中创建与输入样例间的硬链接
  ...
  if (afl->shmem_testcase_mode) { setup_testcase_shmem(afl); }//setup_testcase_shmem()：设置模糊测试输入的共享内存，并通过环境变量__AFL_SHM_FUZZ_ID传递给forkserver
  ...
  /*这里，首先初始化一个，然后在每次出现新的种子簇在进行初始化*/
  if (afl->use_MesFuzz == 1){
    afl->max_op_num = (21 + 1) + (afl->extras_cnt ? 2 : 0) + (afl->a_extras_cnt ? 2 : 0);//afl->extras_cnt为字典中tokens的总数
    mesfuzz_DAR_init(afl, afl->queued_at_start, afl->max_op_num, afl->prob_group_id, cluster_evo_data_x);
  }
  afl->run_times = 0;//总执行次数初始化为0
  ...
  while (likely(!afl->stop_soon)) {
    ...
    do {
      ...
      skipped_fuzz = fuzz_one(afl,cluster_evo_data_x);
...
stop_fuzzing:
  ...
  fclose(afl->mutation_select);
  destroy_queue(afl);
  destroy_extras(afl);
  if (afl->use_seamfuzz != 0 || afl->use_MesFuzz != 0) {
    destroy_seam(afl);
  }
  ...
  free(afl);
  if (cluster_evo_data_x != NULL) free(cluster_evo_data_x);                                                 /* not tracked */
