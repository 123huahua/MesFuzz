...
void check_seamfuzz_dirs(afl_state_t *afl) {
...
void sorting_hit_table(afl_state_t *afl) {
  ...
  for (u32 k = 0; k < (MAP_SIZE << 3); k++) {
    if ((signed) tmp_index >= afl->working_index-1) break;
...
u32 seed_cluster(afl_state_t *afl, u8* in_buf, u32 in_buf_len) {
  ...
  while (checking_group) {
    ...
    //这里
    final_similarity = syntactic_sim * afl->pro_stru_similarity + semantic_sim * (1 - afl->pro_stru_similarity)+ rareness_score;
...
void print_cluster_prob(afl_state_t *afl, u32 seed, u32 group_id,cluster_data_structures_for_AMBES cluster_evo_data_x[],u32 limit)
{ 
	char ss[256]={0};
  sprintf(ss,"%s/seam_prob/prob/%d.txt",afl->out_dir,group_id);
  FILE *prob_fd = fopen(ss, "a");
  if (prob_fd < 0) { PFATAL("Unable to create '%s'", ss); }
  ...
  fprintf(prob_fd, "%llu ", ((afl->prev_run_time + get_cur_time() - afl->start_time) / 1000));

	for (uint i = 0; i < limit; i++) {
    double currentParent = v[i];
    fprintf(prob_fd, "%.3f ", v[i]);
	}
	fprintf(prob_fd, "\n");
...
void creat_cluster_prob_file(afl_state_t *afl, u32 seed, u32 group_id,cluster_data_structures_for_AMBES cluster_evo_data_x[],u32 limit)
{ 
  char ss[256]={0};
  sprintf(ss,"%s/seam_prob/prob/%d.txt",afl->out_dir,group_id);
...
void write_time_while_find_paths(afl_state_t *afl, u32 group_id)
{ 
  char ss[256]={0};
  sprintf(ss,"%s/seam_prob/prob/%d_time.txt",afl->out_dir,group_id);
