...
/* Write a modified test case, run program, process results. Handle
   error conditions, returning 1 if it's time to bail out. This is
   a helper function for fuzz_one_seam(). */
u8 __attribute__((hot))
common_fuzz_stuff_seam(afl_state_t *afl, u8 *out_buf, u32 len) {
  ...
  //u32 prev_queued_discovered = afl->queued_discovered;
  u8 tmp_result;
  //用于解决signal SIGSEGV: address not mapped to object (fault address: 0x100000000)
  unsigned char*p = afl->covered_path_dir;
  int is_zero = 1;
  /*if (p != NULL){
    for (int i = 0;i < 3;i++){
      if (p[i] != 0) is_zero = 0;
  ...
  tmp_result = save_if_interesting(afl, out_buf, len, fault);
  //到这里
  afl->current_hit_cnt = 0;
  update_hit_table(afl);

  afl->queued_discovered += tmp_result;
  afl->group_perf[afl->prob_group_id] += tmp_result;

  if (afl->is_havoc == 1 && afl->is_splice == 0) {

    if (tmp_result == 1) update_success_info(afl);
    else if (afl->use_seamfuzz == 2) {                    // naiveseamfuzz which updates success and failure information based on AFL++ criteria
      update_fail_info(afl);                              // In all cases when generating unintersting test-cases, update failure info.
    } else {
      tmp_result = check_hit_table(afl);
      if (tmp_result == 1) update_success_info(afl);
      else if (tmp_result == 2) update_fail_info(afl);
    }
  }
