...
/* Hit table structure for seamfuzz*/
struct hit_table {
  u32 index;
  u8 bin_index;
  u32 hit_cnt;
...
/* Cluster table structure for seamfuzz */
struct cluster_table {
  u8* file_buf;
  u8* group_covered_path;
  u32 group_id;
  u32 file_id;
  u32 len;
  
  //u8 group_coverage[MAP_SIZE];

  struct cluster_table *next_candidate;
};


/* # of starting point (or partition) for seamfuzz */
#define sp_num 10
...
#define FOREIGN_SYNCS_MAX 32U
  ...
  /* Paramteres/values used for MesFuzz 这里*/

  s32 use_MesFuzz;
  s32 use_MesFuzz_debuging;
  s32 rare_multiple;
  float pro_stru_similarity;
  u64 run_times;//记录一共执行过多少个测试用例
  FILE *mutation_select;

  /* Paramteres/values used for seamfuzz */

  s32 use_seamfuzz;   
  s32 use_seamfuzz_debuging;  
  ...
  u32 group_exists[MAP_SIZE];

  struct hit_table real_hit_table[(MAP_SIZE << 3)];
  struct hit_table rare_hit_table[MAP_SIZE];
  struct hit_table common_hit_table[MAP_SIZE];
  ...
  s32 working_index;
  s32 current_working_index;
  u32 current_hit_cnt;

  u8 make_new_group;

  u8 use_existing_prob;      // 0 -> do not use updated prob. 1 -> use the updated prob
  u32 explore_prob;          // initialize to 30 when seamfuzz starts... 
  ...
  /* probability for grouping option */
  u32 success_op_prob[3000][80];
  u32 success_sp_prob[3000][sp_num];

  u32 fail_op_prob[3000][80];
  u32 fail_sp_prob[3000][sp_num];

  u32 tmp_succ_op_prob[80];
  u32 tmp_succ_sp_prob[sp_num];

  u32 tmp_fail_op_prob[80];
  u32 tmp_fail_sp_prob[sp_num];

  u32 group_path_len[3000];

  u8 *seam_prob_dir,                   /* Working & output directory       */
     *current_prob_dir,          /* currently using prob directory - includes the src_seed_id_number*/
     *prob_dir,
     *swarm_dir,
     *group_covered_path_dir,
     *covered_path_dir;          /* currently using prob directory - covered paths directory*/

  u8 *option_file;        
  
  u8 group_start;

  u32 current_working_int_id;

  struct cluster_table *file_table,
                       *file_table_ori;

  u8 src_file_covered_path[MAP_SIZE];
  u8 group_covered_paths[MAP_SIZE];
  u8 current_seed_path[MAP_SIZE];
  
  double rareness_score,
         current_rareness,
         syn_sim,
         sem_sim;

  u64 reward_update_time;

  u32 thompson_op[3000][80];
  u32 thompson_sp[3000][sp_num];

  u32 current_mut_index;
  u32 reward;
  u32 reward_update_value;
  u32 group_perf[3000];
  u32 group_cnt[3000];
  u32 group_success[30000];//原本是3000,但是运行时间长了会超出导致错误
  u32 group_fail[3000];
  s32 havoc_trials;
...
//这里。用于打印突变概率分布
//int print_cluster_prob(u32 seed, u32 group_id,cluster_data_structures_for_AMBES cluster_evo_data_x[],u32 limit);
typedef struct {
	...
	double ***_parents;
	double ***_children;
	double **_currentRelativeProb;
}cluster_data_structures_for_AMBES;

/* Seam Function */
/* Init functions */
void setup_mesfuzz_dirs(afl_state_t *afl);
void setup_mesfuzz_dirs(afl_state_t *afl);
u8   common_fuzz_stuff_seam(afl_state_t *, u8 *, u32);
void init_group_path(afl_state_t* afl);
//void init_group_path_test(afl_state_t* afl, struct cluster_table *cluster);
void read_covered_path(afl_state_t* afl, u8 *);
//void read_group_covered_path(afl_state_t* afl, u8 *);
void write_covered_path(afl_state_t *, u8 *);
void write_group_path(afl_state_t* afl, u32 group_id);

/* Seamfuzz: functions related to mutation/learning */
u32 select_starting_point(afl_state_t *afl, u32);
u32 select_operator (afl_state_t *afl, u32);


u32 calculate_starting_point(afl_state_t *afl, u32 end_point, u32 index);
u32 seed_cluster(afl_state_t *afl, u8 *, u32);

void update_success_info(afl_state_t *);
void update_fail_info(afl_state_t *);
void update_tmp_succ_group_prob_info(afl_state_t *afl, u32);
void update_tmp_fail_group_prob_info(afl_state_t *afl, u32);
void update_prob_using_group(afl_state_t *afl, s32, u8);


/* functions related to hit table*/
...
void print_updated_probability(afl_state_t *afl, u32);
...
u8   common_fuzz_stuff_param(afl_state_t *, u8 *, u32);
